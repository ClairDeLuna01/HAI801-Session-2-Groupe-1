...
minimaxret minimax(GameState &state, const playerID &playerToWIn, int depth = 0)
{
    ...
    */
    ...
    if(
        // (state.currentPLayer.is_playerO() && playerToWIn.is_playerO()) ||
        // (state.currentPLayer.is_playerX() && playerToWIn.is_playerX())
        state.currentPLayer.value == playerToWIn.value
    )
    {
        m = -1e3;
        for(int i = 0; i < lim; i++)
